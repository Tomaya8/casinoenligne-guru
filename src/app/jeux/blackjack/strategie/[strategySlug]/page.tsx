...
import { allStrategies, getStrategyBySlug } from "@/data/blackjack-strategies";
...
export async function generateStaticParams() {
  return allStrategies.map((s) => ({ strategySlug: s.slug }));
}
...
export default async function StrategyPage({
  ...
  const related = allStrategies.filter((s) => s.slug !== strategySlug);
        ...
        <div className="relative rounded-2xl overflow-hidden mb-8 h-64 sm:h-80">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8">
            <div className="flex items-center gap-2 mb-3">
              <Badge variant="primary">Stratégie Blackjack</Badge>
          ...
          <article>
            ...
            <div className="grid grid-cols-3 gap-3 mb-8">
              ...
              <div className="bg-background-card rounded-xl border border-border p-4 text-center">
                ...
                <p className="text-xs text-foreground-muted">Légalité</p>
                <p className="font-bold text-foreground text-sm">{strategy.legalStatus}</p>
              </div>
            </div>

            <p className="text-lg text-foreground-muted mb-8">{strategy.excerpt}</p>

            {/* Sections with rules + pros/cons integrated */}
          ...
          <div className="sticky top-24 space-y-6">
            <div className="bg-background-card rounded-xl border border-border p-6">
              ...
              <dl className="space-y-3 text-sm">
                ...
                <div className="flex justify-between"><dt className="text-foreground-muted">Légalité</dt><dd className="text-foreground font-medium">{strategy.legalStatus}</dd></div>
              </dl>
            </div>
            ...
            <div className="bg-background-card rounded-xl border border-border p-6">
              ...
              <ul className="space-y-2 text-sm">
                ...
                <li><a href="#rules" className="text-foreground-muted hover:text-accent-primary transition-colors">Étapes Clés</a></li>
            ...
            <div className="bg-background-card rounded-xl border border-border p-6">
              ...
              <div className="space-y-3">
                {related.map((s) => (
                  <Link key={s.slug} href={`/jeux/blackjack/strategie/${s.slug}`} className="flex items-center justify-between group">
                    <div>
                      <span className="text-sm text-foreground group-hover:text-accent-primary transition-colors">{s.name}</span>
                      <span className="block text-xs text-foreground-muted">{s.difficulty}</span>
                    </div>
            ...
            <Link href="/jeux/blackjack" className="flex items-center gap-2 p-4 rounded-xl bg-gradient-to-r from-accent-primary/10 to-accent-secondary/10 border border-accent-primary/20 hover:border-accent-primary/40 transition-colors text-sm font-medium text-foreground">
