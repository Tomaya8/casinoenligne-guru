...
import { allPokerStrategies, getPokerStrategyBySlug } from "@/data/poker-strategies";
...
export async function generateStaticParams() {
  return allPokerStrategies.map((s) => ({ strategySlug: s.slug }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ strategySlug: string }> }): Promise<Metadata> {
  ...
  const strategy = getPokerStrategyBySlug(strategySlug);
  if (!strategy) return {};
  return {
...
export default async function PokerStrategyPage({ params }: { params: Promise<{ strategySlug: string }> }) {
  const { strategySlug } = await params;
  const strategy = getPokerStrategyBySlug(strategySlug);
  if (!strategy) notFound();

  const related = allPokerStrategies.filter((s) => s.slug !== strategySlug);

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <Breadcrumbs items={[{ label: "Jeux", href: "/jeux" }, { label: "Poker", href: "/jeux/poker" }, { label: strategy.name }]} />
        ...
        <div className="relative rounded-2xl overflow-hidden mb-8 h-64 sm:h-80">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8">
            <div className="flex items-center gap-2 mb-3">
              <Badge variant="gold">Stratégie Poker</Badge>
              <Badge variant="primary">{strategy.difficulty}</Badge>
            </div>
          ...
          <article>
            <div className="grid grid-cols-3 gap-3 mb-8">
              ...
              <div className="bg-background-card rounded-xl border border-border p-4 text-center">
                ...
                <p className="font-bold text-foreground text-sm">{strategy.applicability}</p>
              </div>
            </div>
                  ...
                  {i === 0 && (
                    <section id="steps" className="mt-10 scroll-mt-24">
                      <h2 className="text-2xl font-bold text-foreground mb-4 flex items-center gap-2">
                        <BookOpen className="w-5 h-5 text-accent-primary" /> Étapes Clés
                      </h2>
          ...
          <div className="sticky top-24 space-y-6">
            <div className="bg-background-card rounded-xl border border-border p-6">
              ...
              <dl className="space-y-3 text-sm">
                ...
                <div className="flex justify-between"><dt className="text-foreground-muted">Application</dt><dd className="text-foreground font-medium">{strategy.applicability}</dd></div>
              </dl>
            </div>
            <div className="bg-background-card rounded-xl border border-border p-6">
              <h3 className="font-semibold mb-4">Sommaire</h3>
              <ul className="space-y-2 text-sm">
                {strategy.sections.map((section, i) => (<li key={i}><a href={`#section-${i}`} className="text-foreground-muted hover:text-accent-primary transition-colors">{section.heading}</a></li>))}
                <li><a href="#steps" className="text-foreground-muted hover:text-accent-primary transition-colors">Étapes Clés</a></li>
            ...
            <div className="bg-background-card rounded-xl border border-border p-6">
              ...
              <div className="space-y-3">
                {related.map((s) => (
                  <Link key={s.slug} href={`/jeux/poker/strategie/${s.slug}`} className="flex items-center justify-between group">
                    <div><span className="text-sm text-foreground group-hover:text-accent-primary transition-colors">{s.name}</span><span className="block text-xs text-foreground-muted">{s.difficulty}</span></div>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "Article", headline: strategy.title, description: strategy.excerpt, author: { "@type": "Organization", name: "casinoenligne.guru" }, publisher: { "@type": "Organization", name: "casinoenligne.guru" } }) }} />
      {strategy.faqs.length > 0 && (<script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "FAQPage", mainEntity: strategy.faqs.map((faq) => ({ "@type": "Question", name: faq.question, acceptedAnswer: { "@type": "Answer", text: faq.answer } })) }) }} />)}
    </div>
  );
}
