...
import { allRouletteVariants, getRouletteVariantBySlug } from "@/data/roulette-variants";
...
export async function generateStaticParams() {
  return allRouletteVariants.map((v) => ({ variantSlug: v.slug }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ variantSlug: string }> }): Promise<Metadata> {
  ...
  const variant = getRouletteVariantBySlug(variantSlug);
  if (!variant) return {};
  return {
    title: variant.title,
...
export default async function RouletteVariantPage({ params }: { params: Promise<{ variantSlug: string }> }) {
  const { variantSlug } = await params;
  const variant = getRouletteVariantBySlug(variantSlug);
  if (!variant) notFound();

  const related = allRouletteVariants.filter((v) => v.slug !== variantSlug).slice(0, 4);

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <Breadcrumbs items={[{ label: "Jeux", href: "/jeux" }, { label: "Roulette", href: "/jeux/roulette" }, { label: variant.name }]} />
      ...
      {variant.headerImage.startsWith("http") && (
        <div className="relative rounded-2xl overflow-hidden mb-8 h-64 sm:h-80">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8">
            <Badge variant="secondary" className="mb-3">Roulette</Badge>
          ...
          <article>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
              ...
              <div className="bg-background-card rounded-xl border border-border p-4 text-center">
                ...
                <p className="text-xs text-foreground-muted">Numéros</p>
                <p className="font-bold text-foreground">{variant.numbers}</p>
              </div>
              <div className="bg-background-card rounded-xl border border-border p-4 text-center">
            ...
            {/* Sections with rules, pros/cons, comparison integrated */}
            <div className="space-y-10">
              {variant.sections.map((section, i) => (
                <div key={i}>
                  ...
                  {/* Comparison table after 3rd section */}
          ...
          <div className="sticky top-24 space-y-6">
            <div className="bg-background-card rounded-xl border border-border p-6">
              ...
              <dl className="space-y-3 text-sm">
                {[
                  ...
                  { label: "Numéros", value: variant.numbers },
                  { label: "Limites", value: variant.tableLimit },
                ].map((item) => (
            ...
            {related.length > 0 && (
              <div className="bg-background-card rounded-xl border border-border p-6">
                ...
                <div className="space-y-3">
                  {related.map((v) => (
                    <Link key={v.slug} href={`/jeux/roulette/${v.slug}`} className="flex items-center justify-between group">
                      <div>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        headline: variant.title, description: variant.excerpt,
